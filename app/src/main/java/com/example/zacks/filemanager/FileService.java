...
public class FileService extends Service {
    ...
    public static final String FILE_SEARCH_COMPLETED = "FILE_SEARCH_COMPLETED";
    ...
    class FileHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            //当用户点击了取消搜索则不发生广播
            if (!MainActivity2.isComeBackFromNotification) {//继续搜索
    ...
    private void initFileArray(File file) {
        ...
        if (file.canRead()) {
            ...
            for (File currentArray : mFileArray) {
                if (currentArray.getName().contains(SearchBroadCast.mServiceKeyword)) {
                    if (m == -1) {
                        m++;
                        // 返回搜索之前目录
                        mFileName.add("BacktoSearchBefore");
                        mFilePaths.add(MainActivity2.mCurrentFilePath);
                    }
                ...
                if (currentArray.exists() && currentArray.isDirectory()) {
                    //如果用户取消了搜索，应该停止搜索的过程
                    if (MainActivity2.isComeBackFromNotification) {
                        return;
                    }
    ...
    private void fileSearchNotification() {
        ...
        intent.putExtra("notification",
                "当通知还存在，说明搜索未完成，" +
                        "可以在这里触发一个事件，当点击通知回到Activity之后，可以弹出一个框，提示是否取消搜索!");
        PendingIntent mPI = PendingIntent.getBroadcast(this, 0, intent, 0);
        mNotification.setContentIntent(mPI);
